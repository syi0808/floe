...
from task_agent.task_core import TaskStatus  # For status validation
...
class TaskAgent(BaseAgent):
        ...
        while i < len(args):
            ...
            if arg_lower == "priority":
                ...
                if i + 1 < len(args):
                    ...
                    if status_val not in TaskStatus.__args__:
                        return {}, f"Error: Invalid status value '{status_val}'. Allowed: {', '.join(TaskStatus.__args__)}."
    ...
    def _handle_create_task(self, args: List[str], user_id: str) -> str:
        """Handles 'add task' or 'create task' commands."""
        if not args:
            return "Error: No task description provided. Usage: add task \"<description>\" [priority <num>] [due <date>] [tags <tag>]"
        ...
        try:
            ...
            return f"Task '{task.description}' created with ID: {task.id}"
        except ValueError as e:
            return f"Error creating task: {e}"
        except Exception as e:
            return f"An unexpected error occurred while creating the task: {e}"
    ...
    def _format_task_details(self, task: core.TaskItem) -> str:
        ...
        lines.append(f"  Updated: {task.updated_at.strftime('%Y-%m-%d %H:%M:%S %Z') if task.updated_at else 'N/A'}")
        ...
        lines.append(f"  Project Tag: {task.project_tag if task.project_tag else 'N/A'}")
        return "\n".join(lines)

    def _handle_get_task(self, args: List[str], user_id: str) -> str:
        if not args:
            return "Error: Task ID missing. Usage: get task <task_id>"
        task_id_str = args[0]
        try:
            # Validate UUID, though core.get_task will also do this
            uuid.UUID(task_id_str)
        except ValueError:
            return f"Error: Invalid Task ID format '{task_id_str}'."

        task = core.get_task(task_id=task_id_str, user_id=user_id) # Assuming get_task also needs user_id for auth
        if task:
            return self._format_task_details(task)
        else:
            return f"Task with ID '{task_id_str}' not found or not authorized for user '{user_id}'."

    def _handle_update_task(self, args: List[str], user_id: str) -> str:
        if not args:
            return "Error: Task ID missing. Usage: update task <task_id> [description \"...\"] [priority <num>] ..."
        ...
        update_args = args[1:]
        if not update_args:
            return "Error: No update parameters provided. Usage: update task <task_id> description \"...\""
        ...
        if not updates:
             return "Error: No valid fields to update were specified."
        ...
        try:
            ...
            if updated_task:
                ...
                return f"Task with ID '{task_id_str}' not found or failed to update."
        except ValueError as e: # e.g. task not found from core
            return f"Error updating task: {e}"
        except Exception as e:
            return f"An unexpected error occurred while updating task: {e}"

    def _handle_complete_task(self, args: List[str], user_id: str) -> str:
        if not args:
            return "Error: Task ID missing. Usage: complete task <task_id>"
        task_id_str = args[0]
        try:
            uuid.UUID(task_id_str)
        except ValueError:
        ...
        try:
            updated_task = core.update_task(task_id=task_id_str, user_id=user_id, updates=updates)
            if updated_task:
                return f"Task '{updated_task.id}' marked as complete.\n{self._format_task_details(updated_task)}"
            else:
                return f"Task with ID '{task_id_str}' not found or failed to complete."
        except ValueError as e:
            return f"Error completing task: {e}"
        except Exception as e:
            return f"An unexpected error occurred while completing task: {e}"

    def _handle_delete_task(self, args: List[str], user_id: str) -> str:
        if not args:
            return "Error: Task ID missing. Usage: delete task <task_id>"
        task_id_str = args[0]
        try:
            uuid.UUID(task_id_str)
        except ValueError:
        ...
        if core.delete_task(task_id=task_id_str, user_id=user_id): # Assuming delete_task also needs user_id
            return f"Task '{task_id_str}' deleted successfully."
        else:
            # core.delete_task might raise an error for not found, or return False
            return f"Task with ID '{task_id_str}' not found or not authorized for deletion."

    def _handle_list_tasks(self, args: List[str], user_id: str) -> str:
        ...
        while i < len(args): # Basic parser for list filters
            ...
            if arg == "status":
                if i + 1 < len(args):
                    ...
                    if status_filter not in TaskStatus.__args__ and status_filter is not None: # Allow None
                         return f"Error: Invalid status value '{status_filter}'. Allowed: {', '.join(TaskStatus.__args__)}."
                    i += 1
                else:
                    return "Error: Status value missing for list command."
            elif arg == "tag" or arg == "project":
                if i + 1 < len(args):
                    project_tag_filter = args[i+1]
                    i += 1
                else:
                    return "Error: Tag value missing for list command."
            else:
                 return f"Error: Unknown filter '{args[i]}' for list command. Try 'status <val>' or 'tag <val>'."
            i += 1

        try:
            tasks = core.list_tasks(user_id=user_id, status=status_filter, project_tag=project_tag_filter)
            if not tasks:
            ...
            for task in tasks:
                ...
                if task.project_tag:
                    line += f", Tag: {task.project_tag}"
                response_lines.append(line)
            return "\n".join(response_lines)
        except Exception as e:
            return f"An unexpected error occurred while listing tasks: {e}"

    def process(self, request: str, user_id: str) -> str:
        try:
