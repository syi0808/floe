...
func (console *Console) serveAuthority(writer http.ResponseWriter, request *http.Request, scope clientScope) {
	...
	authorityTransport := AuthorityHandler{ProducerMetadata: console.producerMetadata}
	if engine != nil {
...
// Request decoding for authority endpoints. Reading and bounding the request is
// transport; what the proof means belongs to the authorization owner.
...
func strictAuthorityJSON(data []byte) bool { return StrictJSON(data) }
