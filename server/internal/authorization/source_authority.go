...
func (console *Console) WithCurrentSource(principal Principal, reference SourceReference, consume func(SourceSnapshot) error) error {
	if consume == nil {
	...
	if record.ConnectorID == "calendar.google" || record.ConnectorID == "calendar.microsoft" {
		...
		return fenceRuntime.WithVerifiedProviderIdentity(record.Credential, record.ProviderIdentity, func() error {
			snapshot := SourceSnapshot{SourceReference: reference, PersonID: record.PersonID, Active: true}
