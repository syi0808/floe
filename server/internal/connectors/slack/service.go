...
func NewService(client *Client, channel, thread string) (*Service, error) {
	if client == nil || !channelPattern.MatchString(channel) || thread != "" && !threadPattern.MatchString(thread) {
		return nil, ErrInvalidInput
	}
	return &Service{client: client, channel: channel, thread: thread, clock: time.Now}, nil
}
...
func (service *Service) ReadWorkContextView(ctx context.Context) (common.WorkContextView, error) {
	...
	view, err := service.client.WorkContext(ctx, service.channel, service.thread, service.clock())
	if err == nil {
		service.last = &view
	}
	return view, err
...
func (service *Service) ConnectionSnapshot(ctx context.Context) (any, error) {
	...
	defer service.operation.Unlock()
	now := service.clock()
	view, err := service.client.WorkContext(ctx, service.channel, service.thread, now)
	if err == nil {
		service.last = &view
	...
	snapshot := common.Snapshot{Descriptor: ConnectorDescriptor(), Connection: common.Connection{SchemaVersion: 1, ConnectorID: "slack.conversations", State: state, ObservedAtUnixMS: observed, LastFailure: &common.Failure{Kind: kind, ObservedAtUnixMS: observed}}, Views: []common.ViewSnapshot{}}
	if service.last != nil {
