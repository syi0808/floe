...
func New(store Store, config Config) (*Runtime, error) {
	...
	return &Runtime{store: store, config: config, client: &http.Client{Transport: transport, CheckRedirect: func(*http.Request, []*http.Request) error { return http.ErrUseLastResponse }}, authURL: defaultAuthURL, tokenURL: defaultTokenURL, callbackAddress: "127.0.0.1:0"}, nil
}
...
func (runtime *Runtime) Token(ctx context.Context) (string, error) {
	...
	form := url.Values{"grant_type": {"refresh_token"}, "client_id": {runtime.config.ClientID}, "refresh_token": {current.RefreshToken}, "scope": {"offline_access " + mailReadScope}}
...
func (runtime *Runtime) Action(ctx context.Context, action string) (any, error) {
	...
	return map[string]any{"status": status, "auth_url": authURL, "scope": mailReadScope}, nil
}
...
func (runtime *Runtime) startLogin() error {
	...
	parameters := url.Values{"client_id": {runtime.config.ClientID}, "response_type": {"code"}, "redirect_uri": {redirectURI}, "scope": {"offline_access " + mailReadScope}, "state": {state}, "code_challenge": {base64.RawURLEncoding.EncodeToString(challenge[:])}, "code_challenge_method": {"S256"}, "response_mode": {"query"}}
...
func (runtime *Runtime) callback(flow *loginFlow, writer http.ResponseWriter, request *http.Request) {
	...
	defer cancel()
	form := url.Values{"grant_type": {"authorization_code"}, "client_id": {runtime.config.ClientID}, "code": {request.URL.Query().Get("code")}, "redirect_uri": {flow.redirectURI}, "code_verifier": {flow.verifier}, "scope": {"offline_access " + mailReadScope}}
	if runtime.config.ClientSecret != "" {
		form.Set("client_secret", runtime.config.ClientSecret)
	}
...
func (runtime *Runtime) tokenRequest(ctx context.Context, form url.Values, previous *tokenBundle) (*tokenBundle, error) {
	...
	if !validCredential(refresh, 16384) || !hasScope(scope, mailReadScope) {
		return nil, ErrCredentialExpired
	}
...
func (runtime *Runtime) load() *tokenBundle {
	...
	if json.Unmarshal([]byte(encoded), &value) != nil || value.ClientID != runtime.config.ClientID || !validCredential(value.AccessToken, 16384) || !validCredential(value.RefreshToken, 16384) || !hasScope(value.Scope, mailReadScope) || value.ExpiresAt.IsZero() {
		return nil
	}
	runtime.mu.Lock()
