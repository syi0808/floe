from typing import Optional, Any, Dict, List # Removed TypedDict as AgentResponse is now imported
...
from .common_types import AgentResponse # Import AgentResponse from common_types
...
if __name__ == '__main__':
    ...
    schedule_agent = ScheduleAgent() # Instantiate ScheduleAgent
    engine.register_agent(agent_instance=schedule_agent) # Register ScheduleAgent
    # engine.register_agent(agent_instance=schedule_agent_mock) # Removed MockScheduleAgent to avoid intent collision for 'extract_schedule_info'
    engine.register_agent(agent_instance=task_agent_mock)

    print("\n--- Testing OrchestrationEngine with Registered Agents ---")

    # Test case 1: Schedule intent (handled by actual schedule_agent)
    print("\nTest Case 1: Schedule Intent (Actual ScheduleAgent)")
    schedule_intent_actual = {
        'intent': 'extract_schedule_info', # This intent is supported by ScheduleAgent
        'entities': {'title': 'Project Kickoff', 'participants': ['Dev Team', 'Product Owner'], 'date': 'Next Wednesday', 'time': '11 AM', 'description': 'Initial meeting to discuss project scope.'}
    }
    response_actual = engine.route_request(schedule_intent_actual, 'user_actual_001')
    print(f"Response for actual schedule_intent: {response_actual}")

    # Test case 2: Another schedule intent (This would have been handled by MockScheduleAgent - schedule_agent_v2)
    # Since MockScheduleAgent is not registered, this will now result in 'Unknown intent' or similar.
    print("\nTest Case 2: Update Schedule Intent (Now expecting Unknown Intent)")
    update_schedule_intent = {
        'intent': 'update_schedule_info', # This intent was supported by MockScheduleAgent
        'entities': {'event_id': 'evt_123', 'new_time': '3 PM'}
    }
    response_update = engine.route_request(update_schedule_intent, 'user123')
    print(f"Response for update_schedule_intent: {response_update}")
    ...
    response_task = engine.route_request(task_intent, 'user456')
    print(f"Response for task_intent: {response_task}")
    ...
    response_general = engine.route_request(general_intent, 'user789')
    print(f"Response for general_intent: {response_general}")

    # Test case 5: Unknown intent (not registered)
    print("\nTest Case 5: Unknown Intent")
    unknown_intent = {'intent': 'unknown_action', 'entities': {}}
    response_unknown = engine.route_request(unknown_intent, 'user000')
    print(f"Response for unknown_intent: {response_unknown}")
    ...
    response_rogue = engine.route_request(rogue_intent_data, 'userX')
    print(f"Response for rogue_intent_missing_instance: {response_rogue}")
    ...
    response_fail = engine.route_request(failing_intent_data, 'user_fail')
    print(f"Response for failing_intent: {response_fail}")
