...
4. Schedule now consumes Google and Microsoft through a selected, Person-owned server connector and
   consumes EventKit and Android through a provider/revision/device-bound local Calendar observation
   port. The Flutter producer does not yet publish iOS/Android Calendar batches into that port, so
   the device-native end-to-end path remains incomplete even though the former macOS-only dispatch
   has been removed.
5. Rust and Go now share the bounded `calendar.timeline` request contract: range, cursor, limit and
   optional canonical connector selection. Multi-provider requests fail closed when selection is
   ambiguous, and selected-provider failures do not fall through to another Calendar connector.
...
| **C3 — Provider parity** | Google Calendar, Microsoft Calendar/Mail and Android Calendar/Contacts/Health Connect adapters and lifecycle routes exist. Schedule uses the exact selected Google/Microsoft server connector through the bounded View contract and exposes a common Person/device/provider/revision-bound observation port for EventKit and Android. Android Contacts and derived Wellbeing already publish through local FFI; Calendar publication from Flutter remains unwired. | Go selected-connector and owner-binding tests; Rust request-contract, projected-observation and device-publication tests; Android debug build plus existing Dart/native fixtures. | No recorded live Google/Microsoft OAuth provider run or Android physical-device permission/read run for this gate. | Wire Flutter Calendar batches into the local observation port, then run the full live provider cohort, capture conforming snapshots and demonstrate same-logical-source arbitration/deduplication with real data, physical Android lifecycle transitions and revoked/unsupported states. |
