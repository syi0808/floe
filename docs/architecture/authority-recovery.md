...
- An Expert assignment's persisted `SourceSelectionReference` is configuration identity only: connector, connection, execution owner, capability, resource and contract version. It contains no credential, grant, source-authority revision, payload or processing consent. A Task admits the exact binding selection; current binding drift blocks later execution but never redirects to another source. Completed Task replay returns the historical result without reacquiring evidence.
...
- Context's Manager tool catalogue owns its direct remote View list. App derives Expert Observe consumers only from trusted shipped manifests with an active installed assignment whose binding selects the exact connector, connection, execution owner and resource. An arbitrary installed/supplied extension receives no automatic first-party consumer. App includes `assistant` separately for Manager-advertised direct remote Views and applicable native personal policies; Calendar and unadvertised extensions receive no Manager wildcard. Binding changes neither create nor revoke a grant; a newly selected consumer still needs current Access review.
- App fingerprints each prospective first-party Observe member policy from its exact View, sorted consumer identifiers and categories, purpose and source-processing policy. Connection review echoes that fingerprint through the client and enable recomputes it before mutation. Conversation includes the same opaque fingerprint in each immutable reviewed member and its existing target digest, including reviewed absence; resolution and refresh compare current App policy before mutation. The fingerprint is review identity, not Access grant authority.
...
For delegated Calendar proposals, Actions owns the proposal schema and media type. The saved delegation must match the exact durable Task and contain exactly one Actions proposal artifact; executable-looking package JSON is inert. Vault checks the artifact's exact contributor against Task coverage and admitted selection, the settled Registry instance, assignment, package and invocation, then revalidates current binding and source/grant/policy fences before new publication or dispatch. A source Read grant or proposal artifact is never Act authority. Approval and durable pre-dispatch intent remain separate from publication. Once external intent is durable, later binding changes do not erase it or turn uncertainty into a blind retry; lost-response recovery keeps lookup/reconciliation authoritative.
