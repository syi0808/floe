...
- Context's Manager tool catalogue owns its direct remote View list. App derives exact Expert Observe consumers from the product-shipped bundle's manifest requirements, not from arbitrary installed Registry packages. It includes exact `assistant` only for Manager-advertised direct remote Views and applicable native personal policies; Calendar and unadvertised extensions receive no Manager wildcard.
