...
- Context's Manager tool catalogue owns its direct remote View list. App includes exact `assistant` in first-party Observe consumers only for those advertised Views, alongside the built-in Expert consumers; Calendar and unadvertised extensions receive no Manager wildcard.
