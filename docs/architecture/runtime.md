...
Context's declared-source resolver accepts a key only when it occurs in the admitted manifest and Task selection, bounds the query and returns a generic `RequirementReadOutcome` with every successful contributor dependency. The common Expert host exposes that generic read, model, dependency capture and settlement surface; package code decodes its own typed views. Candidate discovery is a read-only settings operation over current connection metadata and canonical Context source identities; it never reads payload or creates a grant. Product sees opaque candidate IDs and safe labels, not `SourceSelectionReference`, and save resolves IDs against a fresh catalog. Expert reads receive only the Task's exact selected references. Remote acquisition checks the pinned producer and selected connector, connection and resource before grant/current-policy and signed-preview checks; unrelated grants are not candidates or blockers. Native Calendar checks current owner state only to verify the selected connection/resources and authority, never to add calendars. Attention, Contacts and Wellbeing receive exact native references; Tasks and Confirmed Memory validate intrinsic local Context projection targets without Access grants. ConfirmedInteractions currently has no authorized candidate and reports an optional limitation rather than successful empty evidence. Manager direct tools retain separate product-owned current selection. Context retains exact dependency reauthorization for later model/history use.

The Expert Registry owns per-assignment source binding as configuration, not source permission. A required requirement may remain empty while the assignment is callable; its execution produces a durable `ExpertBinding` navigation interaction. Explicit settings use an expected binding revision and trusted outer operation ID; exact lost-ack retries rejoin only while that operation is still the last binding mutation. Initial first-party setup may choose only unambiguous sources, and later connections, startup, admission or inspection never auto-rebind. Disabled assignments stay disabled across default startup ensure. Manifest schema 2 and Registry schema 3 are direct cutovers without old-data decoders; an old development profile must be explicitly recreated rather than silently reset. Context and Access decide source reads at invocation time. First-party Observe consumers are derived only from trusted shipped, active assignments selecting the exact source target; Manager `assistant` is added separately for direct views. Binding never mutates a grant or confers model-recipient or Action authority. Calendar grants retain consumer-policy authority and the reviewed native subject. Package artifact semantics belong to the package; a generic Task result is not a domain schema or an Action proposal. Schedule proposes a bounded interval draft, and trusted settlement binds exactly one captured Calendar dependency to an Actions-owned proposal artifact. The artifact is review evidence, not execution authority.
