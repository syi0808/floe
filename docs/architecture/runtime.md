...
Schedule uses the same production model-execution path as every built-in Expert: `BuiltinExpertEndpoint` → `BuiltinExpertHost` → `ExpertModelHost` → `InferenceExecutor`. The shipped bundle owns its `ExpertRegistration` collection (manifest plus runner); App builds dispatch from that collection instead of listing individual Experts. The manifest defines package, definition revision, bounded prompt/result identities and source requirements, while current Registry installation still uses the older setup topology pending the direct Registry cutover. The common runtime prompt role is `Expert`; each package's Role source, revision and content retain its distinct prompt identity. Delegated model inference uses the `experts.delegated` consumer for profile observation and exact-recipient consent, while Calendar/source reads keep the exact package consumer such as `floe.builtin.schedule`. Existing consent for `experts.builtin` does not authorize the new consumer.
