...
Context exposes a signed remote Calendar View read and dependency reauthorization under a selected DataAccessGrant resource, with Access checking the Calendar-specific local-only scope and Vault resolving its consumer-policy binding. The common built-in Expert host injects a Calendar reader that uses that Context read for each currently selected remote resource and records its exact dependency. Context also has a provider-neutral native admission sequence that checks the device stamp, current connection and grant; the Vault adapter selects that grant by connection without an App-supplied setup handle and rejects ambiguous old mappings. Native observation/projection has not yet switched to this sequence, and the separate Schedule endpoint still uses its setup-bound path until deletion.
