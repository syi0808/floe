...
Schedule has one production model-execution path: `ScheduleEndpoint` → `run_calendar_expert_endpoint` → `ExpertModelHost` → `InferenceExecutor`. Calendar source acquisition, exact dependency coverage, grant revalidation and atomic Task/Expert settlement remain with their existing owners. The Vault Task repository no longer has a Schedule settlement owner: it validates a settlement against the selected Task Expert and the Registry assignment/package identity as part of atomic settlement. There is no parallel Schedule Session/agent-turn model runtime. Test-only persisted Delegation/ExpertResult fixtures support proposal inspection and publication tests without another model runtime. The synthetic App AgentFixture runtime and its ABI are deleted. Test-only owner fixtures do not keep a second Conversation runtime alive.

Context owns remote and native Calendar View acquisition and dependency reauthorization. Remote reads bind a signed source preview to an exact DataAccessGrant resource. Native reads bind the current connection, EventKit subject stamp, local-only grant, bounded observation and post-read recheck, then retain dependency evidence for later model/history admission. The common built-in Expert host uses these Context paths under the requesting Expert identity even without a server connection. Calendar reads distinguish ready, temporary unavailability and review-required outcomes; native OS permission denial or a grant that excludes the requesting consumer requires review rather than authorizing the read. Optional Experts record missing Calendar context rather than treating it as empty evidence. The separate Schedule endpoint still uses its setup-bound App path until deletion. Vault's native grant lookup can select by connection but still consults the old Registry mapping, which checkpoint 03 will remove.
