...
| 03 | [Generic Registry and runtime registration](03-registry-and-runtime.md) | Complete | Atomic Directory and pinned Task `0ef358da`; exact durable admission and assignment-local settlement `04bcdf5e`; Registry/Flutter DTO `e606f91b`; generic manifest, install, and bundle seams `184cb4ee`, `4acf3c0e`, `b7186a19`, `4086d091`, `7d3545ad`, `8859f0a5`; declared-source host, trusted consumer policy, and Context current selection `22312760`, `d5a6d5f9`; current architecture `a53b608e`. Owner tests, workspace check, serial full workspace test (`CARGO_INCREMENTAL=0 RUST_TEST_THREADS=1`), architecture checker, FFI build, Flutter analyze/test/macOS build, and diff check passed. Parallel Vault/App runs encountered deadline-sensitive test timeouts; the same complete suites passed serially. Persistent source binding remains unimplemented until 04. |
