...
def get_chat_template(model_id, variant=None):
    try:
        ...
        if response.status_code == 401:
            raise Exception(
                "Access to this model is gated, please request access, authenticate with `huggingface-cli login` and make sure to run `pip install huggingface_hub`"
            )
    ...
    if isinstance(chat_template, str):
        ...
        if variant is None:
            if "default" not in variants:
                raise Exception(
                    f"Please specify a chat template variant (one of {format_variants()})"
                )
            variant = "default"
            sys.stderr.write(
                f'Note: picked "default" chat template variant (out of {format_variants()})\n'
            )
        elif variant not in variants:
            raise Exception(
                f"Variant {variant} not found in chat template (found {format_variants()})"
            )
