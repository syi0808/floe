...
let accessRequest: [String: Any] = ["schema_version": 1, "person_id": raw["person_id"]!,
  "provider": "event_kit", "device_id": "fixture-device", "connection_id": "fixture-connection",
  "connection_revision": 1, "calendar_ids": ["second", "first"],
...
for change: [String: Any] in [
  ...
  var touched = false
  do {
    _ = try calendarViewAccess(invalid, permission: { touched = true }, contains: { _ in true }, generation: { "stable" }, subjectFingerprint: { String(repeating: "a", count: 64) })
...
do {
  _ = try calendarViewAccess(accessRequest, permission: { throw NSError(domain: "fixture", code: 1) },
    contains: { _ in queried = true; return true }, generation: { "stable" }, subjectFingerprint: { String(repeating: "a", count: 64) })
