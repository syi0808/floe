...
mod tests {
    ...
    #[derive(Default)]
    struct TestKeyState {
        values: Mutex<HashMap<(floe_kernel::PersonId, Uuid), [u8; 32]>>,
        blocked: AtomicBool,
        calls: AtomicUsize,
    ...
    fn request(person_id: floe_kernel::PersonId) -> (GrantSourceBinding, GrantScope) {
    ...
    async fn owner_and_key_binding_fail_closed() {
        let root = root();
        let person_id = floe_kernel::PersonId::new();
        let other_person = floe_kernel::PersonId::new();
        let keys = TestKeys::default();
        let vault = EncryptedAgentVault::create(root.path(), person_id, keys.clone())
            .await
            .unwrap();
    ...
    async fn missing_component_and_cleanup_mismatch_fail_closed() {
        ...
        let missing_person = floe_kernel::PersonId::new();
    ...
    async fn capacity_reopen_and_ciphertext_boundaries_hold() {
        ...
        let sentinel_person = floe_kernel::PersonId::new();
