...
/// Canonical deadline/cancellation fence. Never consults legacy policy.
...
///
/// No legacy `ModelTransportRequest` is constructed here: instructions render
...
/// no second UUID is minted for the same attempt. Polling, deadline,
/// release and failure mapping mirror the legacy path; only the request
/// preparation and the step decoding are canonical.
