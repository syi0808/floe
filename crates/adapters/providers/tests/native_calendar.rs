//! What the macOS calendar source does with a real EventKit fixture.
//!
//! Provider adapters own the native request, the response it decodes and the
//! failures it must not turn into a partial read. Whether a grant admits the
//! read is Access's judgment, and this suite does not repeat it.
...
use floe_context::{CalendarObserveRequest, CalendarSource};
