...
#[cfg(unix)]
impl<Keys: floe_vault::VaultKeyProvider> CapabilityHost for PersistedFixtureCapabilities<'_, Keys> {
...
#[cfg(unix)]
impl<Keys: floe_vault::VaultKeyProvider> InProcessAgent for PersistedFixtureCapabilities<'_, Keys> {
...
struct ExpertSessionStore<'store, Keys> {
    vault: &'store floe_vault::EncryptedAgentVault<Keys>,
    registry: &'store Mutex<AgentRegistry>,
    persisted_revision: AtomicU64,
}

#[cfg(unix)]
impl<Keys: floe_vault::VaultKeyProvider> SessionStore for ExpertSessionStore<'_, Keys> {
...
fn agent_error(error: floe_vault::StoreError) -> AgentFailure {
    match error.code {
