...
use floe_domain::{
    AllDaySchedule, CalendarConnection, CalendarFailure, CalendarProvider, CalendarRange,
    CalendarSelection, CalendarSource, CalendarSyncStatus, Capture, CaptureProcessing,
    CaptureSource, DaySnapshot, DomainError, DomainRef, Event, EventId, EventSchedule, Note,
    NoteId, Priority, SourceRef, Task, TaskId, TimedSchedule, TimelineItem,
};
use thiserror::Error;
use uuid::Uuid;
...
use floe_protocol::{
    ...
    CalendarSelectionDto, CalendarSourceDto, CalendarSyncStatusDto, CaptureDto,
    CaptureProcessingDto, CaptureSourceDto, DaySnapshotDto, DomainRefDto, EventDto,
...
parse_test_id!(parse_person_id, floe_domain::PersonId);
parse_test_id!(parse_capture_id, floe_domain::CaptureId);
parse_id!(parse_event_id, EventId);
parse_id!(parse_task_id, TaskId);
parse_id!(parse_note_id, NoteId);

pub(crate) fn calendar_provider_to_dto(value: CalendarProvider) -> CalendarProviderDto {
    match value {
...
pub(crate) fn calendar_provider_from_dto(value: CalendarProviderDto) -> CalendarProvider {
    match value {
...
pub(crate) fn calendar_scope_to_dto(
    value: floe_domain::CalendarScope,
) -> floe_protocol::CalendarScopeDto {
    match value {
        floe_domain::CalendarScope::Selected => floe_protocol::CalendarScopeDto::Selected,
        floe_domain::CalendarScope::All => floe_protocol::CalendarScopeDto::All,
    }
}

pub(crate) fn calendar_scope_from_dto(
    value: floe_protocol::CalendarScopeDto,
) -> floe_domain::CalendarScope {
    match value {
        floe_protocol::CalendarScopeDto::Selected => floe_domain::CalendarScope::Selected,
        floe_protocol::CalendarScopeDto::All => floe_domain::CalendarScope::All,
    }
}

pub(crate) fn calendar_failure_to_dto(value: CalendarFailure) -> CalendarFailureDto {
    match value {
...
pub(crate) fn calendar_failure_from_dto(value: CalendarFailureDto) -> CalendarFailure {
    match value {
...
pub(crate) fn calendar_selection_to_dto(value: CalendarSelection) -> CalendarSelectionDto {
...
pub(crate) fn calendar_selection_from_dto(value: CalendarSelectionDto) -> CalendarSelection {
    CalendarSelection {
        calendar_id: value.calendar_id,
        calendar_name: value.calendar_name,
    }
...
pub(crate) fn calendar_range_to_dto(value: CalendarRange) -> CalendarRangeDto {
    CalendarRangeDto {
...
pub(crate) fn calendar_range_from_dto(value: CalendarRangeDto) -> CalendarRange {
    CalendarRange {
        start_date: value.start_date,
        end_date_exclusive: value.end_date_exclusive,
        timezone_offset_seconds: value.timezone_offset_seconds,
...
pub(crate) fn calendar_source_to_dto(value: CalendarSource) -> CalendarSourceDto {
    CalendarSourceDto {
...
#[cfg(test)]
pub(crate) fn calendar_source_from_dto(value: CalendarSourceDto) -> CalendarSource {
    CalendarSource {
        can_modify: value.can_modify,
...
pub(crate) fn calendar_sync_status_to_dto(value: CalendarSyncStatus) -> CalendarSyncStatusDto {
...
#[cfg(test)]
pub(crate) fn calendar_sync_status_from_dto(value: CalendarSyncStatusDto) -> CalendarSyncStatus {
    CalendarSyncStatus {
        last_success_at: value.last_success_at,
...
pub(crate) fn calendar_connection_to_dto(value: CalendarConnection) -> CalendarConnectionDto {
...
#[cfg(test)]
pub(crate) fn calendar_connection_from_dto(value: CalendarConnectionDto) -> CalendarConnection {
    CalendarConnection {
        connection_id: value.connection_id,
        device_id: value.device_id,
        disconnected: value.disconnected,
...
pub(crate) fn priority_to_dto(value: Priority) -> PriorityDto {
    match value {
...
pub(crate) fn priority_from_dto(value: PriorityDto) -> Priority {
    match value {
...
pub(crate) fn source_ref_to_dto(value: SourceRef) -> SourceRefDto {
    match value {
...
#[cfg(test)]
pub(crate) fn source_ref_from_dto(
    value: SourceRefDto,
...
pub(crate) fn domain_ref_to_dto(value: DomainRef) -> DomainRefDto {
    match value {
...
pub(crate) fn domain_ref_from_dto(
    value: DomainRefDto,
...
pub(crate) fn event_schedule_to_dto(value: EventSchedule) -> EventScheduleDto {
    match value {
...
pub(crate) fn event_schedule_from_dto(
...
pub(crate) fn event_to_dto(value: Event) -> EventDto {
    EventDto {
...
#[cfg(test)]
pub(crate) fn event_from_dto(value: EventDto) -> Result<Event, ProtocolConversionError> {
    ...
    let mut event = Event::new(person_id, value.title, schedule, source, created_at)?;
    event.id = parse_event_id(&value.id, "id")?;
    event.updated_at = parse_timestamp(&value.updated_at, "updated_at")?;
    event.revision = floe_domain::Revision(value.revision);
...
pub(crate) fn task_to_dto(value: Task) -> TaskDto {
    TaskDto {
        id: value.id.to_string(),
        person_id: value.person_id.to_string(),
        title: value.title,
...
#[cfg(test)]
pub(crate) fn task_from_dto(value: TaskDto) -> Result<Task, ProtocolConversionError> {
    let person_id = parse_person_id(&value.person_id, "person_id")?;
    let deadline = value
        .deadline
        .as_deref()
    ...
    task.revision = floe_domain::Revision(value.revision);
...
pub(crate) fn note_to_dto(value: Note) -> NoteDto {
    NoteDto {
        id: value.id.to_string(),
        person_id: value.person_id.to_string(),
...
#[cfg(test)]
pub(crate) fn note_from_dto(value: NoteDto) -> Result<Note, ProtocolConversionError> {
    let person_id = parse_person_id(&value.person_id, "person_id")?;
    let source = source_ref_from_dto(value.source)?;
    let created_at = parse_timestamp(&value.created_at, "created_at")?;
    let mut note = Note::new(person_id, value.content, source, created_at)?;
    note.id = parse_note_id(&value.id, "id")?;
    note.updated_at = parse_timestamp(&value.updated_at, "updated_at")?;
    note.revision = floe_domain::Revision(value.revision);
...
pub(crate) fn timeline_item_to_dto(value: TimelineItem) -> TimelineItemDto {
    match value {
...
#[cfg(test)]
pub(crate) fn timeline_item_from_dto(
...
pub(crate) fn capture_source_to_dto(value: CaptureSource) -> CaptureSourceDto {
    match value {
...
#[cfg(test)]
pub(crate) fn capture_source_from_dto(value: CaptureSourceDto) -> CaptureSource {
    match value {
...
pub(crate) fn capture_processing_to_dto(value: CaptureProcessing) -> CaptureProcessingDto {
    match value {
...
#[cfg(test)]
pub(crate) fn capture_processing_from_dto(
...
pub(crate) fn capture_to_dto(value: Capture) -> CaptureDto {
    CaptureDto {
        id: value.id.to_string(),
        person_id: value.person_id.to_string(),
...
#[cfg(test)]
pub(crate) fn capture_from_dto(value: CaptureDto) -> Result<Capture, ProtocolConversionError> {
    let person_id = parse_person_id(&value.person_id, "person_id")?;
    ...
    capture.revision = floe_domain::Revision(value.revision);
    Ok(capture)
}

pub(crate) fn day_snapshot_to_dto(
    value: DaySnapshot,
...
#[cfg(test)]
pub(crate) fn day_snapshot_from_dto(
