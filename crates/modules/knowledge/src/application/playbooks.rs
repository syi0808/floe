...
#[serde(transparent)]
pub struct PlaybookAudience(pub String);

impl From<&str> for PlaybookAudience {
...
pub struct PlaybookIndexEntry {
    ...
    pub roles: Vec<PlaybookAudience>,
}
...
impl PlaybookRegistry {
    ...
    pub fn begin(
        &self,
        role: impl Into<PlaybookAudience>,
    ) -> Result<PlaybookSession, AgentFailure> {
        let role = role.into();
...
fn validate_playbook(playbook: &Playbook) -> Result<(), AgentFailure> {
    ...
    if index.reference.id.trim().is_empty()
        ...
        || index
            .roles
            .iter()
            .any(|role| role.0.trim().is_empty() || role.0.len() > 128)
        || index.roles.contains(&PlaybookAudience::from("learner"))
...
mod tests {
    ...
    fn playbook(id: &str, parent: Option<&str>, children: &[&str]) -> Playbook {
        Playbook {
            index: PlaybookIndexEntry {
                ...
                roles: vec![PlaybookAudience::from("schedule_expert")],
            },
    ...
    fn nested_summaries_are_hidden_until_the_parent_is_loaded() {
        ...
        let mut session = registry
            .begin(PlaybookAudience::from("schedule_expert"))
            .unwrap();
    ...
    fn learner_role_cannot_receive_playbooks() {
        ...
        learner_playbook.index.roles = vec![PlaybookAudience::from("learner")];
