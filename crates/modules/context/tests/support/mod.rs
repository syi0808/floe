//! An in-memory [`TimelineRepository`] for Context's tests.
//!
//! Context projects Day's stored timeline into bounded views. These tests give
//! it a timeline directly so that what they assert is the projection and its
//! budgets, not a database. Storage itself is the Vault adapter's test to make,
//! and this map can be put into states a healthy store would never return.
...
struct State {
    ...
    /// Who each row is filed under, which a store indexes independently of the
    /// payload it holds. The two can disagree; a reader must not trust either
    /// one alone.
    task_owners: HashMap<TaskId, PersonId>,
    note_owners: HashMap<NoteId, PersonId>,
    unreadable: bool,
}
...
impl TestTimelineRepository {
    ...
    /// File `value` under `owner` whatever the payload says, so a reader can be
    /// shown the row a healthy store would never hold: one whose index and
    /// payload name different people.
    pub fn force_put_task(&self, owner: PersonId, value: &Task) {
        let mut state = self.state().expect("timeline");
        state.tasks.insert(value.id, value.clone());
        state.task_owners.insert(value.id, owner);
    }

    pub fn force_put_note(&self, owner: PersonId, value: &Note) {
        let mut state = self.state().expect("timeline");
        state.notes.insert(value.id, value.clone());
        state.note_owners.insert(value.id, owner);
    }

    /// Fail every read, the way an unreadable store does.
    pub fn fail_reads(&self) {
        self.state().expect("timeline").unreadable = true;
    }
}
...
impl TimelineRepository for TestTimelineRepository {
    ...
    async fn put_task(&self, value: &Task) -> Result<(), DayError> {
        let mut state = self.state()?;
        state.tasks.insert(value.id, value.clone());
        state.task_owners.insert(value.id, value.person_id);
        Ok(())
    }
    ...
    async fn put_note(&self, value: &Note) -> Result<(), DayError> {
        let mut state = self.state()?;
        state.notes.insert(value.id, value.clone());
        state.note_owners.insert(value.id, value.person_id);
        Ok(())
    }
    ...
    async fn list_tasks(&self, person_id: PersonId) -> Result<Vec<Task>, DayError> {
        ...
        if state.unreadable {
            return Err(DayError::storage("unreadable timeline"));
        }
        Ok(state
            .tasks
            .values()
            .filter(|task| state.task_owners.get(&task.id) == Some(&person_id))
            .cloned()
            .collect())
    }
    ...
    async fn list_notes(&self, person_id: PersonId) -> Result<Vec<Note>, DayError> {
        ...
        Ok(state
            .notes
            .values()
            .filter(|note| state.note_owners.get(&note.id) == Some(&person_id))
            .cloned()
            .collect())
    }
    ...
    async fn classify(&self, capture: &Capture, item: &TimelineItem) -> Result<(), DayError> {
        ...
        match item {
            ...
            TimelineItem::Task(task) => {
                ...
                state.task_owners.insert(task.id, task.person_id);
            }
            TimelineItem::Note(note) => {
                state.notes.insert(note.id, note.clone());
                state.note_owners.insert(note.id, note.person_id);
            }
        }
