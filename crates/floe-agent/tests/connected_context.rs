...
fn go_microsoft_mail_descriptor_conforms_to_the_shared_rust_contract() {
    ...
    .unwrap();
    assert!(validate_connector_snapshot(&snapshot, 1_789_128_000_000).is_empty());
    assert_eq!(snapshot.descriptor.provider, "microsoft");
    assert_eq!(snapshot.descriptor.execution, ExecutionLocation::Server);
    assert_eq!(snapshot.descriptor.views[0].id, "mail.communication");
    assert!(
        snapshot
            .descriptor
            .capabilities
...
fn go_google_calendar_descriptor_conforms_to_the_shared_rust_contract() {
    ...
    .unwrap();
    assert!(validate_connector_snapshot(&snapshot, 1_789_128_000_000).is_empty());
    assert_eq!(snapshot.descriptor.id, "calendar.google");
    assert_eq!(snapshot.descriptor.provider, "google_calendar");
    assert_eq!(snapshot.descriptor.views[0].id, "calendar.timeline");
    assert!(
        snapshot
            .descriptor
            .capabilities
...
fn go_microsoft_calendar_descriptor_conforms_to_the_shared_rust_contract() {
    ...
    .unwrap();
    assert!(validate_connector_snapshot(&snapshot, 1_789_128_000_000).is_empty());
    assert_eq!(snapshot.descriptor.id, "calendar.microsoft");
    assert_eq!(snapshot.descriptor.provider, "microsoft_calendar");
    assert_eq!(snapshot.descriptor.views[0].id, "calendar.timeline");
    assert!(
        snapshot
            .descriptor
