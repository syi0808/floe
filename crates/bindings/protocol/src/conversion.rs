//! Codec between the app wire DTOs and the owner domain values.
//!
//! The wire shape belongs here; owners never speak DTOs internally.

use chrono::{DateTime, NaiveDate, SecondsFormat, Utc};
use floe_day::{AllDaySchedule, CalendarConnection, CalendarFailure, CalendarProvider, CalendarRange, CalendarSelection, CalendarSource, CalendarSyncStatus, Capture, CaptureProcessing, CaptureSource, DaySnapshot, DomainError, DomainRef, Event, EventSchedule, Note, Priority, SourceRef, Task, TimedSchedule, TimelineItem};
use floe_kernel::{EventId, NoteId, TaskId};
use thiserror::Error;
use uuid::Uuid;
...
parse_test_id!(parse_person_id, floe_kernel::PersonId);
parse_test_id!(parse_capture_id, floe_kernel::CaptureId);
...
pub fn calendar_scope_to_dto(
    value: floe_day::CalendarScope,
) -> crate::CalendarScopeDto {
    match value {
        floe_day::CalendarScope::Selected => crate::CalendarScopeDto::Selected,
        floe_day::CalendarScope::All => crate::CalendarScopeDto::All,
    }
}

pub fn calendar_scope_from_dto(
    value: crate::CalendarScopeDto,
) -> floe_day::CalendarScope {
    match value {
        crate::CalendarScopeDto::Selected => floe_day::CalendarScope::Selected,
        crate::CalendarScopeDto::All => floe_day::CalendarScope::All,
    }
}
...
pub fn event_from_dto(value: EventDto) -> Result<Event, ProtocolConversionError> {
    ...
    event.revision = floe_kernel::Revision(value.revision);
...
pub fn task_from_dto(value: TaskDto) -> Result<Task, ProtocolConversionError> {
    ...
    task.revision = floe_kernel::Revision(value.revision);
...
pub fn note_from_dto(value: NoteDto) -> Result<Note, ProtocolConversionError> {
    ...
    note.revision = floe_kernel::Revision(value.revision);
...
pub fn capture_from_dto(value: CaptureDto) -> Result<Capture, ProtocolConversionError> {
    ...
    capture.revision = floe_kernel::Revision(value.revision);
    Ok(capture)
}
...
mod tests {
    ...
    use floe_day::{CalendarConnection, CalendarFailure, CalendarProvider, CalendarRange, CalendarScope, CalendarSelection, CalendarSyncStatus, Capture, CaptureSource, DaySnapshot, DomainRef, Event, EventSchedule, Note, Priority, SourceRef, Task, TimedSchedule, TimelineItem};
use floe_kernel::{CaptureId, EventId, PersonId};
    use crate::{CalendarProviderDto, DaySnapshotDto, EventScheduleDto, SourceRefDto};
    use uuid::Uuid;
