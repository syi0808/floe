...
fn native_calendar_grants_without_authority_are_rejected() {
    ...
    assert!(AgentRegistry::restore(snapshot, registry.instance_id()).is_err());
}
...
fn setup_is_one_revision_default_off_and_uses_the_builtin_schedule_catalog() {
    ...
    for (provider, data_class) in [
        ...
        registry
            .install_calendar_expert(person, &next, &packaging())
            .unwrap();
        assert_eq!(registry.snapshot().packages.len(), 2);
...
fn invalid_scope_capacity_and_package_collision_leave_no_partial_setup() {
    ...
    registry
        .install_calendar_expert(person, &first, &packaging())
        .unwrap();
...
fn corrupt_setup_receipts_fail_restore() {
    ...
    for mode in 0..11 {
        ...
        match mode {
            ...
            9 => {
                invalid.calendar_views[0].connection_scope =
