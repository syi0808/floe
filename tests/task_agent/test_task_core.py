...
def test_taskitem_status_validation():
    ...
    with pytest.raises(ValidationError):
        task_core.TaskItem(user_id="user1", description="Invalid status", status="pending")

    task_valid_status = task_core.TaskItem(user_id="user1", description="Valid status", status="in-progress")
...
def test_taskitem_serialization():
    ...
    assert isinstance(task_dict["id"], UUID) # model_dump keeps original types if not specified by json_encoders target
    ...
    task_json_dict = task.model_dump(mode='json') # Pydantic v2, mode='json' applies json_encoders

    assert isinstance(task_json_dict["id"], str)
    assert task_json_dict["created_at"] == now.isoformat().replace("+00:00", "Z") # Pydantic v2 default for datetimes
    assert task_json_dict["due_date_utc"] == (now + timedelta(days=1)).isoformat().replace("+00:00", "Z")
...
def test_update_task_existing():
    ...
    assert updated_task.created_at == original_created_at # created_at should not change
...
def sample_tasks():
    ...
    now = datetime.now(timezone.utc)
    # User 1 tasks
    task1_u1 = task_core.create_task("user1", "U1 Task 1 (Todo, Prio 2)", priority=2, status="todo", project_tags=["t1","common"])
    task2_u1 = task_core.create_task("user1", "U1 Task 2 (In-Prog, Prio 1)", priority=1, status="in-progress", due_date_utc=now + timedelta(days=2), project_tags=["t2","common"])
    task3_u1 = task_core.create_task("user1", "U1 Task 3 (Done, Prio 3)", priority=3, status="done", due_date_utc=now - timedelta(days=1))

    # User 2 tasks
    task1_u2 = task_core.create_task("user2", "U2 Task 1 (Todo, Prio 2)", priority=2, status="todo", project_tags=["t1"])
    ...
    task_core._task_storage[str(task1_u1.id)] = task1_u1 # Re-store if create_task doesn't allow created_at override

    task5_u1 = task_core.TaskItem(user_id="user1", description="U1 Task 5 (Todo, Prio 2, Newer)", priority=2, status="todo", created_at=now)
...
def test_list_tasks_by_user_id(sample_tasks):
    ...
    assert user1_tasks_retrieved[0].description == "U1 Task 2 (In-Prog, Prio 1)" # Prio 1
    assert user1_tasks_retrieved[1].description == "U1 Task 1 (Todo, Prio 2)"     # Prio 2, older
    assert user1_tasks_retrieved[2].description == "U1 Task 5 (Todo, Prio 2, Newer)" # Prio 2, newer
...
def test_list_tasks_by_project_tag(sample_tasks):
    ...
    non_existent_tag_tasks = task_core.list_tasks(user_id="user1", project_tag="non_existent_tag")
...
def test_list_tasks_by_due_date(sample_tasks):
    ...
    past_due_tasks = task_core.list_tasks(
        user_id="user1",
        due_date_end=now - timedelta(hours=1) # Any time before now
    )
    ...
    far_future_tasks = task_core.list_tasks(
        user_id="user1",
        due_date_start=now + timedelta(days=10)
    )
...
def test_list_tasks_combined_filters(sample_tasks):
    ...
    filtered_tasks = task_core.list_tasks(user_id="user1", status="todo", project_tag="t1")
...
def test_list_tasks_sorting(sample_tasks):
    ...
    st1 = task_core.TaskItem(user_id="sort_test_user", description="ST1 Prio2 Second", priority=2, created_at=time_now)
    task_core._task_storage[str(st1.id)] = st1

    st2 = task_core.TaskItem(user_id="sort_test_user", description="ST2 Prio1 First", priority=1, created_at=time_now + timedelta(seconds=1)) # Prio 1, should be first
    task_core._task_storage[str(st2.id)] = st2

    st3 = task_core.TaskItem(user_id="sort_test_user", description="ST3 Prio2 First", priority=2, created_at=time_now - timedelta(seconds=1)) # Prio 2, older, should be after Prio 1
    task_core._task_storage[str(st3.id)] = st3

    st4 = task_core.TaskItem(user_id="sort_test_user", description="ST4 Prio3 Last", priority=3, created_at=time_now + timedelta(seconds=2))
...
def test_list_tasks_empty_result():
    ...
    no_match_tag = task_core.list_tasks(user_id="user_with_one_task", project_tag="nonexistent")
