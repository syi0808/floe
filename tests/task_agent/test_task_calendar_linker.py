...
# --- Test Data ---
SAMPLE_START_TIME = datetime.now(timezone.utc) + timedelta(
    days=1
...
class TestCreateCalendarEventFromTask(unittest.TestCase):
    ...
    def test_create_new_event_generates_uuid_event_id(self):
        ...
        self.assertTrue(
            self._is_uuid(calendar_event.event_id),
            "Generated event_id should be a UUID.",
        )
    ...
    def test_create_event_uses_base_event_id_if_provided(self):
        ...
        calendar_event = create_calendar_event_from_task(
            task_input, base_event_id=custom_event_id
        )
...
class TestTaskCalendarLinker(unittest.TestCase):
    ...
    def test_add_task_to_calendar(self):
        ...
        returned_event_id = self.linker.add_task_to_calendar(
            self.sample_task_input, "primary"
        )

        self.assertIsNotNone(returned_event_id)
        self.assertTrue(
            uuid.UUID(returned_event_id), "Returned ID should be a UUID string."
        )
        ...
        self.assertEqual(
            created_calendar_event_arg.summary, self.sample_task_input.summary
        )
        self.assertEqual(
            created_calendar_event_arg.task_id_ref, self.sample_task_input.task_id
        )

    def test_get_linked_event(self):
        self.linker._connected = True
        mock_event = CalendarEvent(
            event_id=self.floe_event_id,
            summary="S",
        ...
        self.mock_adapter.get_event.assert_called_once_with(
            self.floe_event_id, "primary"
        )
    ...
    def test_update_linked_event(self):
        ...
        success = self.linker.update_linked_event(
            self.floe_event_id, self.sample_task_input, "primary"
        )
        ...
        self.assertEqual(
            updated_calendar_event_arg.event_id, self.floe_event_id
        )  # Ensure it's the same
        self.assertEqual(
            updated_calendar_event_arg.summary, self.sample_task_input.summary
        )
    ...
    def test_remove_task_from_calendar(self):
        ...
        self.assertTrue(success)
        self.mock_adapter.delete_event.assert_called_once_with(
            self.floe_event_id, "primary"
        )
    ...
    def test_list_linked_events(self):
        ...
        events = self.linker.list_linked_events(
            "primary", start_filter, end_filter, "task123"
        )

        self.assertEqual(events, mock_events)
        self.mock_adapter.list_events.assert_called_once_with(
            "primary", start_filter, end_filter, "task123"
        )
    ...
    def test_block_time_for_task_via_schedule_agent(self):
        ...
        mock_schedule.process.return_value = {
            "status": "success",
            "data": {"event_id": "evt123"},
        }
        task_input = TaskInput(**VALID_TASK_DATA_DICT)
        event_id = block_time_for_task_via_schedule_agent(
            mock_schedule, "user1", task_input
        )
    ...
    def test_create_update_delete_flow(self):
        ...
        self.linker._connected = True
        self.mock_adapter.create_event.side_effect = lambda ev, tgt=None: ev.event_id
        self.mock_adapter.update_event.return_value = True
        self.mock_adapter.delete_event.return_value = True

        # Create
        event_id = self.linker.add_task_to_calendar(self.sample_task_input)
        self.assertIsNotNone(event_id)
        self.mock_adapter.create_event.assert_called_once()

        # Update
        success_update = self.linker.update_linked_event(
            event_id, self.sample_task_input
        )
        self.assertTrue(success_update)
        self.mock_adapter.update_event.assert_called_once_with(event_id, ANY, None)

        # Delete
        success_delete = self.linker.remove_task_from_calendar(event_id)
        self.assertTrue(success_delete)
        self.mock_adapter.delete_event.assert_called_once_with(event_id, None)
